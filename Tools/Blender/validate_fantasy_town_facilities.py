...
bpy.ops.wm.open_mainfile(filepath=os.path.join(ROOT, "ArtSource", "Blender", "FantasyTownFacilities.blend"))
with open(os.path.join(REVIEW, "model_manifest.json")) as handle:
    manifest = json.load(handle)
assert {model["name"] for model in manifest["models"]} == {"Tavern", "Clinic", "Guardhouse"}
records = []
