...
bpy.ops.wm.open_mainfile(filepath=os.path.join(ROOT, "ArtSource", "Blender", "FantasyCultureFacilities.blend"))
with open(os.path.join(REVIEW, "model_manifest.json")) as handle:
    manifest = json.load(handle)
assert {model["name"] for model in manifest["models"]} == {"Casino", "Museum", "Arena"}
records = []
...
arena_vertices, arena_faces = [], []
for obj in bpy.data.collections["Arena"].objects:
    if obj.type != "MESH":
        continue
    transform = obj.matrix_parent_inverse @ obj.matrix_basis
    offset = len(arena_vertices)
    arena_vertices.extend(transform @ vertex.co for vertex in obj.data.vertices)
    arena_faces.extend(tuple(offset+i for i in face.vertices) for face in obj.data.polygons)
arena_tree = BVHTree.FromPolygons(arena_vertices, arena_faces)
height = max(v.z for v in arena_vertices)
floor_hit = arena_tree.ray_cast(Vector((0,0,height+1)), Vector((0,0,-1)))
assert floor_hit[0] is not None and abs(floor_hit[0].z-0.35) <= height*2**-23, "open arena sand floor"
front = min(v.y for v in arena_vertices)-1
entrance_hit = arena_tree.ray_cast(Vector((0,front,1.5)), Vector((0,1,0)), -front)
assert entrance_hit[0] is None, "open passage from front gate to arena center"
...
report = {"environment": "Blender " + bpy.app.version_string, "blend_reopen": "PASS",
          ...
          "arena_open_floor": "PASS", "arena_entrance_passage": "PASS"}
