...
bpy.ops.wm.open_mainfile(filepath=os.path.join(ROOT, "ArtSource", "Blender", "FantasyNightlifeBuildings.blend"))
with open(os.path.join(REVIEW, "model_manifest.json")) as handle:
    manifest = json.load(handle)
assert {model["name"] for model in manifest["models"]} == {"CrimsonRowhouse", "VelvetTerrace", "LanternSpire", "VeiledCourtyard"}
records = []
...
courtyard_vertices, courtyard_faces = [], []
for obj in bpy.data.collections["VeiledCourtyard"].objects:
    if obj.type != "MESH":
        continue
    transform = obj.matrix_parent_inverse @ obj.matrix_basis
    offset = len(courtyard_vertices)
    courtyard_vertices.extend(transform @ vertex.co for vertex in obj.data.vertices)
    courtyard_faces.extend(tuple(offset+i for i in face.vertices) for face in obj.data.polygons)
tree = BVHTree.FromPolygons(courtyard_vertices, courtyard_faces)
assert tree.ray_cast(Vector((0,-4.66,1.7)), Vector((0,1,0)), 7.4)[0] is None, "open archway into courtyard"
...
report = {"environment": "Blender " + bpy.app.version_string, "blend_reopen": "PASS",
          "models": records, "external_textures_required": False, "courtyard_archway_open": "PASS"}
