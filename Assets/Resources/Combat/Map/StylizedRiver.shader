Shader "WarSimulation/Stylized River"
{
    Properties
    {
        ...
        _PatternScale ("Pattern size (meters)", Float) = 3.5
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                float2 surfaceWarp = float2(
                    Noise(p * 1.3 + float2(phase * 0.38, -phase * 0.31)),
                    Noise(p * 1.3 + float2(-phase * 0.29, phase * 0.42) + 23.1)) - 0.5;
                float broad = Noise(p * float2(1.2, 2.5) + surfaceWarp * 2.0);
                ...
                float breaks = smoothstep(0.72, 0.9,
                ...
                float smallRipples = smoothstep(0.74, 0.9 + fwidth(flecks), flecks);
                float highlights = smoothstep(0.4, 0.8, waveB);
                half3 color = lerp(_BaseColor.rgb * 0.9, _RippleColor.rgb,
                    broad * 0.48 + ripples * 0.32);
                color = lerp(color, _RippleColor.rgb, ridge * 0.35);
                color = lerp(color, _FoamColor.rgb,
                    ridge * highlights * 0.22 + smallRipples * 0.08);
